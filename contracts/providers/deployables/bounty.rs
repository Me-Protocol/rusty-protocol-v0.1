...
> BountyController for T {
    ...
    fn deposit_bounty(&mut self, reward:AccountId, amount:Balance, requestor: AccountId) -> Result<bool, ProtocolError>{
    ...
    fn withdraw_bounty(&mut self, reward:AccountId, amount:Balance, requestor: AccountId, to: AccountId) -> Result<bool, ProtocolError>{
        ensure_address_is_not_zero_address(reward)?;
        ensure_address_is_not_zero_address(requestor)?;
        ensure_value_is_not_zero(amount)?;
        if !reward_is_bounty_reward(self, reward) {return Err(ProtocolError::RewardIsNotBountyReward)}
    ...
    fn set_trigger_limit(&mut self, reward:AccountId, new_trigger_limit:Balance, requestor: AccountId) -> Result<bool, ProtocolError>{
    ...
    fn get_trigger_limit(&mut self, reward:AccountId, requestor: AccountId) -> Result<u128, ProtocolError>{
        ensure_address_is_not_zero_address(reward)?;
        ensure_address_is_not_zero_address(requestor)?;
        if !reward_is_bounty_reward(self, reward) {return Err(ProtocolError::RewardIsNotBountyReward)}
