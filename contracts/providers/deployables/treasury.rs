...
> TreasuryController for T {
    ...
    fn deposit_reward_and_or_me(&mut self, reward:AccountId, reward_amount:Balance, me_amount:Balance, brand: BRAND_ID_TYPE, requestor: AccountId, metadata: Option<String>) -> Result<bool, ProtocolError>{
        ...
        let (current_reward_balance, current_me_balance) = objectively_obtain_pool_balances(treasury, reward, me);
        if reward_amount > 0 {
            let previous_reward_balance = get_treasury_reward_balance(self, reward);
            if previous_reward_balance > current_reward_balance {return Err(ProtocolError::TreasuryDepositNotRecognized)}
        ...
        if me_amount > 0 {
            ...
            if previous_me_balance > current_me_balance {return Err(ProtocolError::TreasuryDepositNotRecognized)}
    ...
    fn withdraw_reward_and_or_me(&mut self, reward:AccountId, reward_amount:Balance, me_amount:Balance, brand: BRAND_ID_TYPE, to: AccountId, requestor: AccountId) -> Result<bool, ProtocolError>{
        let treasury = Self::env().account_id();
        //emit event
         withdraw_reward_and_or_me(self, reward, reward_amount, me_amount, brand, to, requestor, treasury)?;
        Ok(true)
    }
    ...
    fn top_up_pool_with_reward_and_or_me(&mut self, reward:AccountId, pool_id: AccountId, reward_amount:Balance, me_amount:Balance, brand: BRAND_ID_TYPE, requestor: AccountId, metadata: Option<String>) -> Result<bool, ProtocolError>{
        let treasury = Self::env().account_id();
        //emit event
         withdraw_reward_and_or_me(self, reward, reward_amount, me_amount, brand, pool_id, requestor, treasury)?;
        Ok(true)
    }
...
fn withdraw_reward_and_or_me<T>(instance: &mut T, reward:AccountId, reward_amount:Balance, me_amount:Balance, brand: BRAND_ID_TYPE, to: AccountId, requestor: AccountId, treasury:AccountId) -> Result<bool, ProtocolError>
where T: Storage<TreasuryRecord> 
{
    ...
    let (current_reward_balance, current_me_balance) = objectively_obtain_pool_balances(treasury, reward, me);
    if reward_amount > 0 {
        if current_reward_balance < reward_amount {return Err(ProtocolError::InsufficientTreasuryRewardBalance)}
    ...
    if me_amount > 0 {
        if current_me_balance < me_amount {return Err(ProtocolError::InsufficientTreasuryMeBalance)}
