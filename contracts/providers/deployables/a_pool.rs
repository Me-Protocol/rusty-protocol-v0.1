...
impl<T: Storage<PoolState> + Storage<PoolConfig> + Storage<access_control::Data>> PoolController for T {
  

    default fn pause_conversations(&mut self) -> Result<(), ProtocolError>{
        if !self.data::<PoolState>().active { return Err(ProtocolError::PoolAlreadyInActive)}
        self.data::<PoolState>().active = false;
        Ok(())
    }

    default fn resume_conversations(&mut self) -> Result<(), ProtocolError>{
        if self.data::<PoolState>().active { return Err(ProtocolError::PoolAlreadyActive)}
        self.data::<PoolState>().active = true;
        Ok(())
    }

    #[modifiers(only_role(POOL_MANAGER))]
    default fn give_pool_tokens(&mut self, pool_numerator_amount:Balance, pool_divisor_amount: Balance, to: AccountId) -> Result<(), ProtocolError>{
    let pool = Self::env().account_id();
    let state = *self.data::<PoolState>();
    let (current_reward_amount, current_me_amount, added_reward_amount, added_me_amount) = (validate_give_pool_tokens_request(state.reward, state.me_token, pool, pool_numerator_amount, pool_divisor_amount, state.last_reward_amount, state.last_me_amount)).unwrap();
   ...
   #[modifiers(only_role(POOL_ADMIN))]
    default fn add_pool_manager(&mut self, new_pool_manager: AccountId) -> Result<(), ProtocolError>{
          ...
          if self.data::<access_control::Data>().has_role(POOL_MANAGER, new_pool_manager) {return Err(ProtocolError::AccountAlreadyPoolManager)}
    ...
    default fn remove_pool_manager(&mut self, pool_manager: AccountId) -> Result<(), ProtocolError>{
        ...
        if !self.data::<access_control::Data>().has_role(POOL_MANAGER, pool_manager) {return Err(ProtocolError::AccountIsNotAPoolManager)}
    ...
    default fn provide_pool_state(&self) -> ( bool,
        AccountId,
         AccountId,
         AccountId,
         Balance,
    ...
    default fn provide_pool_config(&self) -> ( Balance,
        u128,
         Balance,
         Balance,
         Balance,
...
fn ensure_r_is_within_acceptable_range(r_sample: u128, maximum_r_limit: u128)-> Result<(), ProtocolError>{
        if r_sample > maximum_r_limit {return Err(ProtocolError::ConversationWillCausePoolToGoOutOfRag)}
        Ok(())
}


fn validate_give_pool_tokens_request(reward:AccountId, me_token: AccountId, pool: AccountId, reward_amount: Balance, me_amount: Balance, 
    last_reward_amount: Balance, last_me_amount:Balance) -> Result<(Balance, Balance, Balance, Balance),ProtocolError>{
      
      let (current_reward_amount, current_me_amount) = objectively_obtain_pool_balances(pool, reward, me_token);
