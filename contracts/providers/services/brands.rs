...
> BrandController for T {
    ...
    default fn register(&mut self, name: Option<String>, online_presence: Option<String>) -> Result<(), ProtocolError>{
         ...
         let concat: Vec<u8> = name.unwrap().into_iter()
    ...
    fn update_global_brand_config(&mut self, brand_config: GlobalBrandConfig, ignore_default:bool ) -> Result<bool, ProtocolError>{
    ...
    let requestor = Self::env().caller();
     ...
     if !ignore_default {
        ...
        let mut previous_config:GlobalBrandConfig = self.data::<BrandRecords>().GlobalConfig.get(&id).unwrap();
    ...
    fn update_reward_config(&mut self, reward: AccountId, reward_config: RewardConfig, ignore_default:bool ) -> Result<bool, ProtocolError>{
     let requestor = Self::env().caller();
    ...
    fn update_details(&mut self, brand_details:EditableBrandDetails, ignore_default:bool ) -> Result<bool, ProtocolError>{
      let requestor = Self::env().caller();
      let id = get_self_id(self, requestor).unwrap();
   ...
   // to consider
    fn update_reward_details(&mut self, reward: AccountId, reward_details:EditableRewardDetails, ignore_default:bool ) -> Result<bool, ProtocolError>{
        let requestor = Self::env().caller();
    ...
    fn add_bounty_manager(&mut self, reward: AccountId, bounty_manager:AccountId ) -> Result<bool, ProtocolError>{
    ...
    fn remove_bounty_manager(&mut self, reward: AccountId, bounty_manager:AccountId ) -> Result<bool, ProtocolError>{
        let requestor = Self::env().caller();
        ensure_address_is_not_zero_address(bounty_manager)?;
        ensure_address_is_not_zero_address(reward)?;
        ensure_is_issuing_brand(self, reward, requestor)?;
    ...
    fn add_pool_manager(&mut self, reward: AccountId, pool_manager:AccountId ) -> Result<bool, ProtocolError>{
    ...
    fn remove_pool_manager(&mut self, reward: AccountId, pool_manager:AccountId ) -> Result<bool, ProtocolError>{
        ensure_address_is_not_zero_address(pool_manager)?;
        ensure_address_is_not_zero_address(reward)?;
        let requestor = Self::env().caller();
        ensure_is_issuing_brand(self, reward, requestor)?;
    ...
    fn integrate_existing_reward(&mut self, reward: AccountId, reward_description_link:Option<String>, read_t_and_c: bool ) -> Result<bool, ProtocolError>{
        ...
        if !read_t_and_c {return Err(ProtocolError::PleaseReadTandC)}
        ...
        if brand_id == DEFAULT_BRAND_ID {return Err(ProtocolError::BrandDoesNotExist)}
    ...
    fn change_optimal_valuation(&mut self, reward: AccountId, new_optimal_valuation:u128, auto_resume_conversations:bool ) -> Result<bool, ProtocolError>{
        ...
        if auto_resume_conversations {APoolRef::resume_conversations(&pool_id, requestor)?;}
      
        Ok(true)
    }


    fn update_pool_configuration(&mut self, reward: AccountId, editable_pool_config:EditablePoolConfig, ignore_default:bool)-> Result<bool, ProtocolError>{
        ensure_address_is_not_zero_address(reward)?;
        let requestor = Self::env().caller();
        ensure_is_issuing_brand(self, reward, requestor)?;
        let pool_id = self.data::<RewardRecords>().Details.get(&reward).unwrap().pool_id;
        if pool_id == ZERO_ADDRESS.into() {return Err(ProtocolError::RewardHasNoPool)}
        APoolRef::change_pool_config_except_r_optimal(&pool_id, editable_pool_config,ignore_default)?;
        Ok(true)
    }
    ...
    fn top_up_pool_balances(&mut self, reward: AccountId, reward_amount:Balance, me_amount:Balance)-> Result<bool, ProtocolError>{
        ensure_address_is_not_zero_address(reward)?;
        if reward_amount == 0 && me_amount == 0  { return Err(ProtocolError::BothDepositsCanNotBeZero)}
        let requestor = Self::env().caller();
        ensure_is_issuing_brand(self, reward, requestor)?;
        let pool_id = self.data::<RewardRecords>().Details.get(&reward).unwrap().pool_id;
        if pool_id == ZERO_ADDRESS.into() {return Err(ProtocolError::RewardHasNoPool)}
    ...
    fn reduce_pool_balances(&mut self, reward: AccountId, position:u128, reward_amount:Balance, me_amount:Balance)-> Result<bool, ProtocolError>{
        ensure_address_is_not_zero_address(reward)?;
        if reward_amount == 0 && me_amount == 0  { return Err(ProtocolError::BothWithdrawalsCanNotBeZero)}
        let requestor = Self::env().caller();
        ensure_is_issuing_brand(self, reward, requestor)?;
        let pool_id = self.data::<RewardRecords>().Details.get(&reward).unwrap().pool_id;
        if pool_id == ZERO_ADDRESS.into() {return Err(ProtocolError::RewardHasNoPool)}
        APoolRef::withdraw_assets_from_position(&pool_id, position, reward_amount, me_amount, requestor, requestor)?;
        Ok(true)
    }
...
pub fn get_self_id<T>(instance: &mut T,requestor: AccountId) -> Result<BRAND_ID_TYPE, ProtocolError> where T: Storage<BrandRecords>  {
...
pub fn ensure_is_issuing_brand<T>(instance: &mut T,reward: AccountId, requestor: AccountId) -> Result<bool, ProtocolError> where T: Storage<RewardRecords> + Storage<BrandRecords> {
    let issuing_brand =  instance.data::<RewardRecords>().Details.get(&reward).unwrap().issuing_brand; 
